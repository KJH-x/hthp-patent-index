...
├── data/
│   ├── patents.json    # 854 件专利结构化元数据（含 v2 回补字段 COUNTRY/ADC/PDF_IMAGE_COUNT）
...
- 数据采集自智慧芽专利平台（2026-08 ~ 2026-09），共 **854** 件结构化记录。
- 源数据与维护规范位于：
...
- 数据校验（发布门槛，锚点随数据量更新）：
  ```powershell
  node scripts/assert-data.mjs --records=854 --isd=426 --fam-total=799 --fam-multi=54 --country=854 --adc=754 --pic=823
  # 退出码 0=通过
  ```
